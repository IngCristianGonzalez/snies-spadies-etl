...
def load_dim_programa_oferta(engine, df):
    ...
    df_antes = len(df)
    df = df.merge(
        dim_programa, on="codigo_snies_del_programa", suffixes=("", "_programa")
    )
    ...
    df_final = df[["programa_id", "institucion_id", "municipio_id"]].drop_duplicates()
    ...
    if len(df_final) > 0:
        df_final.to_sql(
            "tb_dim_programa_oferta", engine, if_exists="append", index=False
        )
