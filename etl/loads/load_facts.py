...
def load_fact_snies(engine, df, tipo):
    ...
    dim_programa = pd.read_sql(
        "SELECT id, codigo_snies_del_programa FROM tb_dim_programa", engine
    )
    dim_institucion = pd.read_sql(
        "SELECT id, codigo_ies FROM tb_dim_institucion", engine
    )
    dim_municipio = pd.read_sql(
        "SELECT id, codigo_municipio FROM tb_dim_municipio", engine
    )
    ...
    dim_tiempo = pd.read_sql("SELECT id, anio, semestre FROM tb_dim_tiempo", engine)

    # 🔥 NORMALIZACIÓN CONSISTENTE (IGUAL QUE EN load_dim_programa_oferta)
    df_tipo["codigo_snies_del_programa"] = (
        df_tipo["codigo_snies_del_programa"].astype(str).str.strip()
    )
    df_tipo["codigo_de_la_institucion"] = (
        df_tipo["codigo_de_la_institucion"].astype(str).str.strip()
    )
    df_tipo["codigo_del_municipio_programa"] = (
        df_tipo["codigo_del_municipio_programa"].astype(str).str.strip()
    )  # SIN zfill

    dim_programa["codigo_snies_del_programa"] = (
        dim_programa["codigo_snies_del_programa"].astype(str).str.strip()
    )
    dim_institucion["codigo_ies"] = (
        dim_institucion["codigo_ies"].astype(str).str.strip()
    )
    dim_municipio["codigo_municipio"] = (
        dim_municipio["codigo_municipio"].astype(str).str.strip()
    )  # SIN zfill
    ...
    print(
        f"   Programas únicos en dim_programa: {dim_programa['codigo_snies_del_programa'].unique()}"
    )
    ...
    if len(sin_programa) > 0:
        print(
            f"   📊 {len(sin_programa)} registros SIN programa_id válido (no en dim_programa)"
        )
    ...
    print(
        f"   institucion_id únicos EN DF: {sorted(df_tipo['codigo_de_la_institucion'].unique())}"
    )
    ...
    print(
        f"   Conteo: {df_tipo['institucion_id'].value_counts().sort_index().to_dict()}"
    )
    ...
    if registros_sin_programa > 0:
        ...
        print(
            f"   institucion_id en registros sin programa: {sin_prog['codigo_de_la_institucion'].unique()}"
        )
    ...
    df_antes = len(df_tipo)
    df_tipo = df_tipo.merge(
        dim_tiempo, on=["anio", "semestre"], suffixes=("", "_tiempo")
    )
    ...
    if registros_sin_oferta > 0:
        ...
        print(
            f"  institucion_id en registros SIN oferta: {sin_oferta['institucion_id'].unique()}"
        )
        for idx, row in (
            sin_oferta[["programa_id", "institucion_id", "municipio_id"]]
            .drop_duplicates()
            .iterrows()
        ):
            print(
                f"    programa_id={row['programa_id']}, institucion_id={row['institucion_id']}, municipio_id={row['municipio_id']}"
            )
    ...
    ].rename(columns={"id_genero": "genero_id", "valor": "cantidad"})

    print(f"\n📊 En df_final:")
    print(f"   institucion_id únicos: {sorted(df_final['institucion_id'].unique())}")
    print(
        f"   Conteo: {df_final['institucion_id'].value_counts().sort_index().to_dict()}"
    )
    ...
    if len(df_final) > 0:
        df_final.to_sql(
            "tb_fact_snies", engine, if_exists="append", index=False, method="multi"
        )
